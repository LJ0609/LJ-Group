...
def train_net(cfg):
    ...
    if cfg.training_stage==1:
        ...
        GCNnet = gcnnet_list[cfg.inference_module_name]      #'dynamic_volleyball'
        model = GCNnet(cfg,config)
        # Load backbone
        if cfg.load_backbone_stage2:
            model.loadmodel(cfg.stage1_model_path)
        elif cfg.load_stage2model:   #加载训练2的模型
    ...
    for epoch in range(start_epoch, start_epoch+cfg.max_epoch):
        ...
        if epoch > cfg.test_interval_epoch:
            ...
            if cfg.training_stage==2:
                # None
                if test_info['activities_acc'] > 94.0:
                    state = {
                        'epoch': epoch,
