...
class Dynamic_volleyball(nn.Module):
    ...
    def __init__(self, cfg,config):
        ...
        self.globall = nn.Linear(1024, 256)
        self.bbox_fc = nn.Sequential(nn.Linear(12800, 1024), nn.Linear(1024, 256))
    ...
    def forward(self,batch_data):
        ...
        # boxes_features2 = boxes_features.reshape(B * T * N, -1)  # ====================================# B*T*N, D*K*K #[120,12800] B:1 T:10 N:12


        boxes_features=boxes_features.reshape(B,T,N,-1)  #B,T,N, D*K*K
        boxes_features_trr=self.bbox_fc(boxes_features)        #torch.Size([1, 10, 12, 256])  #st_input
        # Embedding 
        ...
        graph_boxes_features, ft_infer_MAD = self.DPI(boxes_features)   #  g :torch.Size([1, 10, 12, 1024])
        # print("graph_boxes_features",graph_boxes_features.shape)
        ...
        if self.cfg.backbone == 'res18':
            ...
            boxes_states = self.dropout_global(boxes_states)  #torch.Size([1, 10, 12, 1024])

        boxes_features_tr = self.globall(boxes_states)    #torch.Size([1, 10, 12, 256])  #st_input
        global_token = boxes_features_tr.permute(2, 0, 1, 3).contiguous().reshape(N, B * T, -1).mean(0, keepdim=True)  #st_input
        ...
        actions_scores1, activities_scores1, aux_loss1 = self.head(boxes_features_trr, global_token)
        ...
        # Predict activities
        boxes_states_pooled, _ = torch.max(boxes_states,dim=2)  #torch.Size([1, 10, 1024])  B  T  1024

        # global_token=boxes_states_pooled.reshape(-1,B*T,1024)#===============================
        # global_token=self.globall(global_token)  #torch.Size([1, 10, 256])  #st_input
